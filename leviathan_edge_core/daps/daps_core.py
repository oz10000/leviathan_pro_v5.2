...
class DAPSEngine:
    """
    Motor DAPS Ω con decaimiento exponencial.
    Versión moderna usada por el Orquestador.
    Mantiene estadísticas por símbolo y aplica olvido exponencial
    para adaptarse a cambios de régimen.
    """
    ...
    def step(self, symbol, closes, signal_quality=0.5):
        """
        Actualiza el estado DAPS para un símbolo y devuelve un factor de modulación omega.
        """
    ...
    def get_state(self, symbol):
        """Devuelve una copia del estado DAPS para un símbolo."""
    ...
    def set_state(self, symbol, state):
        """Restaura el estado DAPS desde un snapshot."""
...
class DAPSCore:
    """
    Núcleo DAPS clásico.
    Versión original usada por el RotationalEngine.
    Recibe epsilon (anomalía) y x_hat (expectativa) directamente.
    """
    ...
    def step(self, epsilon, x_hat):
        """
        Actualiza la variable de estado x y la devuelve.
        """
