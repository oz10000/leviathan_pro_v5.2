...
class OrderRouter:
    ...
    def send(self, symbol, direction, size, atr, leverage):
        """Interfaz legacy (sin feedback detallado)."""
        if not self.live:
        ...
        if resp and resp.get("code") == "0":
            ...
            return {"status": "filled", "price": fill_price, "size": size}
    ...
    def send_with_feedback(self, symbol, direction, size, atr, leverage):
        ...
        if resp and resp.get("code") == "0":
            fill_price = float(resp.get("data", [{}])[0].get("fillPx", 0))
            return {"status": "filled", "price": fill_price, "size": size, "latency_ms": latency, "slippage_pct": 0.0}
