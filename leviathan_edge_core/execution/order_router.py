...
class OrderRouter:
    ...
    def send(self, symbol, direction, size, atr, leverage):
        ...
        resp = self.conn.place_order(symbol, side, size, pos_side, leverage=leverage, tp=tp_price, sl=sl_price)
        if resp and resp.get("status") == "filled":
            return {"status": "filled", "price": 0.0, "size": size, "order_id": resp.get("order_id")}
        print(f"[ORDER_FAILED] {resp.get('error') if resp else 'No response'}", flush=True)
    ...
    def send_with_feedback(self, symbol, direction, size, atr, leverage):
        ...
        if resp and resp.get("status") == "filled":
            return {
                "status": "filled", "price": 0.0, "size": size,
                "latency_ms": latency, "slippage_pct": 0.0,
