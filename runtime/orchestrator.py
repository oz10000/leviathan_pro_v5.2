#!/usr/bin/env python3
"""
LEVIATHAN ORCHESTRATOR – RESILIENT & OPERATIONAL RUNTIME
- Escaneo continuo de todos los activos
...
"""
import sys, os, time, json, signal, logging, hashlib, random, traceback
...
# ── Símbolos de fallback para testnet si la API no responde ──
...
class Orchestrator:
        ...
        try:
            ...
            for cycle in range(max_cycles):
                ...
                try:
                    ...
                    self.health.record_event("cycle_fatal_error")
                    # El ciclo fallido no detiene el runtime; se intenta el siguiente
                finally:
                    # Siempre guardar estado y heartbeat tras cada ciclo
                    ...
                    logging.info(f"Cycle {cycle} completed in {cycle_duration:.2f}s | Balance: {self.engine.capital:.2f}")
                    time.sleep(30)
    ...
    def _write_dashboard_state(self, running: bool, error: Optional[str] = None):
        """Escribe el state.json con todos los campos que el dashboard necesita."""
