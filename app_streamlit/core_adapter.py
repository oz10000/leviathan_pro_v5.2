import sys, os, time, requests, pandas as pd, numpy as np
from datetime import datetime; from pathlib import Path
SCRIPT_DIR = Path(__file__).resolve().parent; EDGE_CORE = SCRIPT_DIR.parent / "leviathan_edge_core"
if str(EDGE_CORE) not in sys.path: sys.path.insert(0, str(EDGE_CORE))
from config import Config; from core.feature_engine import compute_features
...
from daps.daps_core import DAPSCore; from daps.daps_equilibrium import DAPSEquilibrium
from analytics.persistence_engine import PersistenceEngine

class CoreAdapter:
    def __init__(self, mode="simulator", initial_capital=None):
        self.mode = mode; capital = initial_capital if initial_capital else Config.INITIAL_CAPITAL
        self.strategies = [ExpansionStrategy(), PullbackStrategy(), ReaccelerationStrategy(), DepressionBreakoutStrategy()]
        self.state = {"balance": capital, "equity": capital, "pnl": 0.0, "position": None, "signal": None,
                      "loop_count": 0, "last_execution": None, "mode": mode, "equity_history": [capital],
                      "oscillators": {}, "trades": []}
        self.mtf_conv = MTFConvergenceEngine(); self.divergence = DivergenceDetector()
        self.entropy = MarketEntropy(); self.imperfect = ImperfectTradeDetector()
        self.leverage_safety = LeverageSafetyEngine(); self.daps = DAPSCore()
        self.daps_equilibrium = DAPSEquilibrium(); self.persistence = PersistenceEngine()
        self.symbols = ["BTC", "ETH", "SOL", "BNB", "XRP", "DOGE"]; self.asset_data = {}
        self.commission = 0.001; self.slippage = 0.0002
    ...
    def _download_public_candles(self, symbol, bar="5m", limit=100):
        ...
        try:
            resp = requests.get(url, timeout=10); payload = resp.json()
            if payload.get("code") != "0" or not payload.get("data"): return pd.DataFrame()
            raw = payload["data"][::-1]; df = pd.DataFrame(raw).iloc[:, :7]
    ...
    def evaluate_signal(self, df):
        ...
        df_feat = compute_features(df.copy()); row5 = df_feat.iloc[-1]
        ...
        price_arr = df_feat["close"].values[-20:]; vol_arr = df_feat["volume"].values[-20:]
        ...
        # NUEVO: solo aceptar señales que superen el umbral mínimo
        if meta < Config.SCORE_THRESHOLD: return None
        atr = row5["atr"]; safe_lev = self.leverage_safety.safe_leverage(6.0, mtf_score, div_score, 0.0, ent)
        sl = row5["close"] - (1 if direction == "LONG" else -1) * Config.SL_ATR * atr
        tp = row5["close"] + (1 if direction == "LONG" else -1) * Config.TP_ATR * atr
        return {"meta": meta, "direction": direction, "strategy": best_strat, "entry": row5["close"],
                "atr": atr, "mtf": mtf_score, "div": div_score, "ent": ent, "leverage": safe_lev, "sl": sl, "tp": tp}

    def run_cycle(self, leverage=None):
        now = datetime.utcnow()
        if self.state["position"] is not None:
            pos = self.state["position"]; sym = pos["symbol"]; df = self.asset_data.get(sym, {}).get("df")
            if df is not None and len(df) > 0:
                ...
                if exit_sig:
                    d = pos["dir"]; pnl = ((exit_price - pos["entry"]) * d * pos["size"] * pos["leverage"] / pos["entry"])
                    pnl -= abs(exit_price - pos["entry"]) * pos["size"] * self.commission
                    pnl -= pos["entry"] * pos["size"] * self.slippage
                    self.state["balance"] += pnl; self.state["pnl"] += pnl; self.state["equity"] = self.state["balance"]
            ...
            self._update_telemetry(now); return self.get_snapshot()
        candidates = []
        for sym in self.symbols:
            df = self.asset_data.get(sym, {}).get("df")
            if df is None or len(df) < 50: continue
            signal = self.evaluate_signal(df)
            if signal: signal["symbol"] = sym; candidates.append(signal)
        if not candidates: self._update_telemetry(now); return self.get_snapshot()
        best = max(candidates, key=lambda x: x["meta"])
        safe_lev = self.leverage_safety.safe_leverage(self._realtime_sharpe(), best.get("mtf", 0.8),
                                                      best.get("div", 0.2), self._current_drawdown(), best.get("ent", 0.5))
        lev = leverage if leverage is not None else safe_lev; size = 0.01
        ...
        self.state["position"] = {"symbol": best["symbol"], "dir": 1 if best["direction"] == "LONG" else -1,
                                  ...
                                  "strategy": best.get("strategy", ""), "entry_time": time.time(),
                                  "be_active": False, "trail_active": False, "sl": sl, "trail_sl": sl, "tp": tp,
                                  "atr_pct_entry": best["atr"] / best["entry"]}
        self.state["signal"] = best["direction"]; self._update_telemetry(now); return self.get_snapshot()

    def get_snapshot(self):
        return {"balance": self.state["balance"], "equity": self.state["equity"], "pnl": self.state["pnl"],
                "position": self.state["position"], "signal": self.state["signal"],
                "loop_count": self.state["loop_count"], "last_execution": self.state.get("last_execution", "--:--:--")}
    ...
    def compound_growth_table(self, start_capitals=None, num_trades_list=None, leverage=None):
        ...
        wr = 0.935; avg_win = 0.40; avg_loss = -0.50
        table = []
        for capital in start_capitals:
            for trades in num_trades_list:
                ...
                table.append({"Start Capital (USDT)": capital, "Trades": trades,
    ...
    def _realtime_sharpe(self):
        ...
        eq = np.array(self.state["equity_history"][-50:]); rets = np.diff(eq) / eq[:-1]
    ...
    def _update_telemetry(self, now):
        ...
        self.state["loop_count"] += 1; self.state["last_execution"] = now.strftime("%H:%M:%S")
